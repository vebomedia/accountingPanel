...
class C4_account extends BaseController
{
    ...
    {
        helper(['Financepanel\c4_account']);
    }

    //--------------------------------------------------------------------
    /**
     * view default page = c4_account 
     * c4_account   
     */
    public function index()
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '405f8f9']);

        $data                 = ['page_title' => lang('c4_account._page_c4_account')];
        $data['breadcrumb'][] = ['url' => financepanel_url('home/index'), 'title' => lang('home.dashboard')];
        $data['breadcrumb'][] = ['title' => lang('c4_account._page_c4_account'), 'class' => 'active'];

        $this->_render('c4_account', $data);
    }
    //--------------------------------------------------------------------
    /**
     * view page = c4_account
     * Account  
     */
    public function c4_account()
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '39809c4']);
     ...
     * @return json
     */
    public function save($form_slug, $c4_account_id = '')
    {
        $data       = $this->request->getPost();
        $validation = \Config\Services::validation();

        //---------------------------------------------------------------------//
        //For form_slug=cash_account pageSlug=c4_account
        if($form_slug === 'cash_account')
        {

            // cash_account form Validations and security
            $validation->setRules([
                'name'          => ['label' => lang('c4_account.name'), 'rules'=>"required|alpha_numeric_space_turkish|max_length[255]"],
            ]);
            ...
            //----------------------------------------------------------------//
            $filterFormFields = ['name', 'c4_account_id'];
        ...
        //---------------------------------------------------------------------//
        //For form_slug=bank_account pageSlug=c4_account
        elseif($form_slug === 'bank_account')
        {

            // bank_account form Validations and security
            $validation->setRules([
                'name'          => ['label' => lang('c4_account.name'), 'rules'=>"required|alpha_numeric_space_turkish|max_length[255]"],
                'bank_name'     => ['label' => lang('c4_account.bank_name'), 'rules'=>"permit_empty|integer|max_length[11]"],
                'bank_branch'   => ['label' => lang('c4_account.bank_branch'), 'rules'=>"permit_empty|max_length[255]|alpha_numeric_space_turkish"],
                'bank_account_no' => ['label' => lang('c4_account.bank_account_no'), 'rules'=>"permit_empty|max_length[255]|alpha_numeric_space_turkish"],
                'iban'          => ['label' => lang('c4_account.iban'), 'rules'=>"permit_empty|max_length[255]|alpha_numeric_space_turkish"],
            ]);
            if ($validation->run($data) === false)
            {
                return $this->fail($validation->getErrors(), 400, lang('home.error_validation_form'));
            ...
            // clear undefined post fields for security..
            //----------------------------------------------------------------//
            $filterFormFields = ['name', 'bank_name', 'bank_branch', 'bank_account_no', 'iban', 'c4_account_id'];
            $data             = array_intersect_key($data, array_flip($filterFormFields));
            //----------------------------------------------------------------//
        ...
        $C4_accountModel = new C4_accountModel();

        try
        {
            $C4_accountModel->save($data);
        }
        catch (\Exception $e)
        {
        ...
        if(empty($c4_account_id))
        {
            $c4_account_id = $C4_accountModel->getInsertID();
        }

        return $this->respondCreated(['id' => $c4_account_id, 'message' => lang('home.saved')]);
    }
 
    //--------------------------------------------------------------------

    /**
     * Read c4_account data
     ...
     * 
     */
    public function readC4_account($pageSlug)
    {
        ...
        if($pageSlug === 'c4_account')
        {
            $select_text       = "c4_account.*";
            $multipleFields    = [];
            ...
            if (!empty($search_text))
            {
                ...
                $model->groupStart();
                $model->orLike('c4_account.name', $search_text);
                $model->groupEnd();
            }
        ...
        if (!empty($filterArray) && is_array($filterArray))
        {
            foreach ($filterArray as $filterField => $filterValue)
            {
                ...
                {
                    $model->where("c4_account.$filterField", trim(filter_var($filterValue, FILTER_SANITIZE_STRING)));
                }
            }
        }
        ...
        {
            $model->where('c4_account.status', '1');
        }
        else
        {
            $model->where('c4_account.status', $filterArray['status']);
        }
        ...
        {
            $model->where('c4_account.deleted_at', NULL);
        }
        ...
        if (!empty($dateRangeStart) && is_array($dateRangeStart))
        {
            foreach ($dateRangeStart as $key_field => $fieldValue)
            {
                if (in_array($key_field, $table_colons) && !empty($fieldValue))
                {
                    ...
                    $model->where("c4_account.$key_field >=", $fieldValue);
                }
            }
        }
        ...
        if (!empty($dateRangeEnd) && is_array($dateRangeEnd))
        {
            foreach ($dateRangeEnd as $key_field => $fieldValue)
            {
                ...
                {
                    $fieldValue = date( "Y-m-d", strtotime(trim($fieldValue)));
                    $model->where("c4_account.$key_field <=", $fieldValue .' 23:59:59');
                }
            }
        }
        //---------------------------------------------------------------------//
    ...
    public function delete($id)
    {
        ...
        $C4_accountModel = new C4_accountModel();
        if ($C4_accountModel->delete($id, false) === false)
        {
            log_message('error', "Error: $id ID C4_accountModel Delete Error");
            return $this->fail($C4_accountModel->errors());
        }
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        $data['page_title'] = lang('c4_account._form_' .$formSlug);
        $data['formData'] = $_POST;
        ...
        if($formSlug === 'cash_account')
        {
            $data['breadcrumb'][] = ['url' => financepanel_url('c4_account/c4_account'), 'title' => lang('c4_account._page_c4_account')];
            $data['breadcrumb'][] = ['title' => lang('c4_account._form_cash_account'), 'class' => 'active'];
        }
        elseif($formSlug === 'bank_account')
        {
            $data['breadcrumb'][] = ['url' => financepanel_url('c4_account/c4_account'), 'title' => lang('c4_account._page_c4_account')];
            $data['breadcrumb'][] = ['title' => lang('c4_account._form_bank_account'), 'class' => 'active'];
        }
        else
        {
        ...
        if (!empty($id))
        {
            ...
            $data['formData'] += $C4_accountModel->find($id) ?? [];
        }
        ...
        if (!empty($copy))
        {
            ...
            $data['formData'] += $C4_accountModel->find($copy);
            
            if(isset($data['formData']['c4_account_id']))
            {
                unset($data['formData']['c4_account_id']);
            }
        }
    ...
    //--------------------------------------------------------------------
    /**
     * Update single field of c4_account 
     * 
     * @param int $id
     * @return mix
     */
    public function update($id)
    {
        ...
        $batchList = ['account_type', 'currency', 'status', 'deleted_at'];
        ...
        {
            if (!$this->validate([
                'account_type' => ['label' => lang('c4_account.account_type'), 'rules' => 'required|in_list[cash,bank]|alpha_dash'],
            ]))
            {
        ...
        {
            if (!$this->validate([
                'currency' => ['label' => lang('c4_account.currency'), 'rules' => 'required|alpha_numeric_space|max_length[255]|in_list[TRY,USD,EUR,GBP]'],
            ]))
            {
                return $this->fail($this->validator->getErrors(), 400, lang('home.error_invalid_id'));
            }
        ...
        {
            if (!$this->validate([
                'status' => ['label' => lang('c4_account.status'), 'rules' => 'required|integer|max_length[1]|in_list[1,2]'],
            ]))
            {
                return $this->fail($this->validator->getErrors(), 400, lang('home.error_invalid_id'));
            }
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => '238e9fe']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'financepanel/Language/' . $locale . '/c4_account.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'financepanel/Language/' . $conf->defaultLocale . '/c4_account.php';
        ...
        echo 'var LANG_c4_account = ' . json_encode($langArray, JSON_PRETTY_PRINT);
    }
    ...
    private function _render($page, $data = [])
    {
        ...
        $data['table_name']  = 'c4_account';

        if ($this->request->isAJAX())
        {
            try
            {
                echo financepanel_view('c4_account/' . $page, $data);
            }
            catch (\Exception $e)
            {
