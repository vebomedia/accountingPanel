...
class Company extends BaseController
{
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'f5e6807']);
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'c7d9121']);
    ...
    public function showForm($formSlug, $id = null)
    {
        ...
        if($formSlug === 'company')
        {
            $data['breadcrumb'][] = ['url' => financepanel_url('company/company'), 'title' => lang('company._page_company')];
   ...
   //--------------------------------------------------------------------
    ...
    {
        $this->response->setCache(['max-age' => 300, 's-maxage' => 900, 'etag' => 'dc46e9a']);

        $conf   = new \Config\App();
        $locale = $this->request->getLocale();

        //Load Home Lang File
        $langFile = ROOTPATH . 'financepanel/Language/' . $locale . '/company.php';

        if (!file_exists($langFile))
        {
            //Check default lang file
            $langFile = ROOTPATH . 'financepanel/Language/' . $conf->defaultLocale . '/company.php';
    ...
    private function _render($page, $data = [])
    {
        ...
        if ($this->request->isAJAX())
        {
            try
            {
                echo financepanel_view('company/' . $page, $data);
            }
            catch (\Exception $e)
            {
