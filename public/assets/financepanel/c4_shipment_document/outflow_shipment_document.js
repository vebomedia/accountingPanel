...
var outflow_shipment_document = function () {

    var tableID = 'table_outflow_shipment_document';
    var page_slug = 'outflow_shipment_document';
    var tableUrlExt = ''; //Datatabel Url Extension
    var form_sizes = {"outflow_shipment_document":"lg"};
    ...
    var get_url = function (param) {
        ...
        return panel_url('c4_shipment_document' + param);
    ...
    //Module Lang
    var c4_shipment_documentLang = function (param, paramsub) {
        return getLang('c4_shipment_document', param, paramsub);
    };  
    ...
    var init_datatable = function () {
        ...
        DT1 = $('#' + tableID).DataTable({
            ...
            "ajax": {
                ...
                "type": 'POST',
                "data": function (d) {
                    d.formFilter = $('#form_outflow_shipment_document').serialize();
            ...
            columns: [
                {
                    "searchable": false,
                    "targets": 0,
                    title: '<label for="outflow_shipment_documentselectAll" class="text-center" style="width: 100%"><div class="text-center"><input type="checkbox" class="selectAll" id="outflow_shipment_documentselectAll"/></div></label>',
                    defaultContent: '',
                    "data": null,
                    orderable: false,
                    className: 'select-checkbox',
            ...
            {   
                ...
                name: 'description',
                title: c4_shipment_documentLang('description'),
            ...
            {   
                ...
                name: 'address',
                title: c4_shipment_documentLang('address'),
                data: function ( row, type, set, meta ) {
            ...
            {   
                sortable : true,
                name: 'inflow',
                title: c4_shipment_documentLang('inflow'),
                className: "text-center",
                data: function ( row, type, set, meta ) {
                
                var data_list = c4_shipment_documentLang('list_inflow');
            ...
            {   
                sortable : true,
                name: 'issue_date',
                title: c4_shipment_documentLang('issue_date'),
                data: function ( row, type, set, meta ) {
                ...
                sortable : true,
                name: 'shipment_date',
                title: c4_shipment_documentLang('shipment_date'),
                data: function ( row, type, set, meta ) {

                    if(!isEmpty(row.shipment_date) && typeof row.shipment_date === 'string' && row.shipment_date != '0000-00-00' && row.shipment_date != '0000-00-00 00:00:00'){

                        var dateX = moment(row.shipment_date, "YYYY-MM-DD HH:mm:ss");
                        var datenow = moment();
                        ...
                        var colorClass = '';
                            ...
                            return '<span class="' + colorClass + '" title="' + escapeHtml(row.shipment_date) +'">'  +  moment(row.shipment_date, 'YYYY-MM-DD HH:mm:ss').format('LL') + '</span>';
            ...
            },
                    {
                        ...
                        data: function (row) {
                            var id = row.c4_shipment_document_id;
                            var $link = ``;

                                var $link =`<div class="btn-group" role="group" aria-label="">`;
        
                            //outflow_shipment_document form button
                            if(row.inflow === '0' ){

                            $link += `<a
                               href="` + get_url('showForm/outflow_shipment_document/' + id) + `" 
                               data-modalsize="lg"
                               data-datatable="table_outflow_shipment_document"
                               data-modalurl="` + get_url('showForm/outflow_shipment_document/' + id) + `"
                               data-modaldata='{"c4_shipment_document_id":"` + id + `"}'
                               data-action="openformmodal"
                               data-modalview="centermodal"
                               data-modalbackdrop="true"
                               class="btn btn-sm btn-primary"
                               title="` + c4_shipment_documentLang('_form_outflow_shipment_document') + `" 
            ...
            "rowCallback": function (row, data) {
                ...
                .on('select', function (e, dt, type, indexes) {
                    ...
                    $('#batch_outflow_shipment_document').collapse('show');
                    if (type === 'row') {
                ...
                .on('deselect', function (e, dt, type, indexes) {
                    ...
                    if (type === 'row') {
                        ...
                        if (count === 0) {
                            $('#batch_outflow_shipment_document').collapse('hide');
                        }
                ...
                .on('draw', function () {
                    ...
                    var $selectAll = $('#outflow_shipment_documentselectAll');
        ...
        //Form
        $('#form_outflow_shipment_document').find('.generalSearch').bind("keyup search", function () {
            console.log('Searching..');
            DT1.ajax.reload();
        });

        $('#form_outflow_shipment_document').find('.formSearch').on('change', function (e) {
    ...
    return {
        ...
        returnLang: function (param, paramsub) {
            return c4_shipment_documentLang(param, paramsub);
        },
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if ($('#table_outflow_shipment_document').length > 0) {
    
        general.loadPackage('dataTable', function () {
            loadDatatableLang(function () {
                outflow_shipment_document.init_datatable();
            });
        });
