...
var inflow_stock_movement = function () {

    var tableID = 'table_inflow_stock_movement';
    var page_slug = 'inflow_stock_movement';
    var tableUrlExt = ''; //Datatabel Url Extension
    var form_sizes = {"stock_movement86369026":"lg"};
    ...
    var init_datatable = function () {
        ...
        DT1 = $('#' + tableID).DataTable({
            ...
            "ajax": {
                ...
                "type": 'POST',
                "data": function (d) {
                    d.formFilter = $('#form_inflow_stock_movement').serialize();
            ...
            columns: [
                {
                    "searchable": false,
                    "targets": 0,
                    title: '<label for="inflow_stock_movementselectAll" class="text-center" style="width: 100%"><div class="text-center"><input type="checkbox" class="selectAll" id="inflow_stock_movementselectAll"/></div></label>',
                    defaultContent: '',
                    "data": null,
                    orderable: false,
                    className: 'select-checkbox',
                ...
                data: function ( row, type, set, meta ) {
...

                    {
                        ...
                        data: function (row) {
                            ...
                            //stock_movement86369026 form button
                            if(row.inflow === '1' ){

                            $link += `<a
                               href="` + get_url('showForm/stock_movement86369026/' + id) + `" 
                               data-modalsize="lg"
                               data-datatable="table_inflow_stock_movement"
                               data-modalurl="` + get_url('showForm/stock_movement86369026/' + id) + `"
                               data-modaldata='{"c4_stock_movement_id":"` + id + `"}'
                               data-action="openformmodal"
                               data-modalview="centermodal"
                               data-modalbackdrop="true"
                               class="btn btn-sm btn-primary"
                               title="` + c4_stock_movementLang('_form_stock_movement86369026') + `" 
            ...
            "rowCallback": function (row, data) {
                ...
                .on('select', function (e, dt, type, indexes) {
                    ...
                    $('#batch_inflow_stock_movement').collapse('show');
                    if (type === 'row') {
                ...
                .on('deselect', function (e, dt, type, indexes) {
                    ...
                    if (type === 'row') {
                        ...
                        if (count === 0) {
                            $('#batch_inflow_stock_movement').collapse('hide');
                        }
                ...
                .on('draw', function () {
                    ...
                    var $selectAll = $('#inflow_stock_movementselectAll');
        ...
        //Form
        $('#form_inflow_stock_movement').find('.generalSearch').bind("keyup search", function () {
            console.log('Searching..');
            DT1.ajax.reload();
        });

        $('#form_inflow_stock_movement').find('.formSearch').on('change', function (e) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if ($('#table_inflow_stock_movement').length > 0) {
    
        general.loadPackage('dataTable', function () {
            loadDatatableLang(function () {
                inflow_stock_movement.init_datatable();
            });
        });
